...
export default async (req, context) => {
    try {
        ...
        if (fetchError) {
            console.error('Error checking for existing IP:', fetchError);
            return new Response(JSON.stringify({ message: 'Error checking for existing IP', error: fetchError }), {
                status: 500,
        ...
        const { error: insertError } = await supabase
            .from('debt')
            .insert({ amount: -4, description: `Removed by User`, user_ip: userIp });
        ...
        return new Response(JSON.stringify({ message: 'Debt Removed Successfully, Thank You!' }), {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
        });
